...
const TrainerProfile = ({ setSearchQuery, setCurrentPage }) => {
...
const D3RadarChart = ({ entry1Data, entry2Data }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">Trainer Performance Comparison</h4>
...
const ReportTable = ({ tableData, title, currentPage, setCurrentPage, totalPages, onSelectRow }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">{title || "Trainer Report Table"}</h4>
        <Table responsive className="align-items-center">
          <thead>
            <tr>
              {[
                "Trainer", "Runners", "Runs", "Winners", "Wins", "WinPercent_", 
...
const ComparisonTable = ({ entry1, entry2 }) => {
  if (!entry1) return null; // Prevent rendering if no Trainer is selected
        ...
        <Table bordered responsive>
          <thead>
            <tr>
              ...
              <th className="text-center">{entry2 ? entry2.Sire : "Entry 2 (Select a Trainer)"}</th>
            </tr>
          </thead>
          <tbody>
...
const TrainerRadar = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/trainer_name_profile?${params.toString()}`);
        ...
        <Row>
          <Col>
            <Card className="shadow border-0 p-4">
              <CardBody>
                <h3 className="text-center mb-4">Trainer Profile Search</h3>
                ...
                <FormGroup>
                  ...
                  >
                    <option value="entry1">Trainer 1</option>
                    <option value="entry2">Trainer 2</option>
                  </Input>
                </FormGroup>

                {/* Search Box */}
                <TrainerProfile setSearchQuery={setSearchQuery} setCurrentPage={setCurrentPage} />
                ...
                {/* Display Radar Chart & Comparison Table when a Trainer is selected */}
                {selectedEntry1 && (
                  <Row className="mt-4">
                    <Col md={6}>
