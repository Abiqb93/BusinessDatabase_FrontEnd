...
const D3RadarChart = ({ entry1Data, entry2Data }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">Dam Performance Comparison</h4>
...
const ReportTable = ({ tableData, title, currentPage, setCurrentPage, totalPages, onSelectRow }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">{title || "Dam Report Table"}</h4>
        <Table responsive className="align-items-center">
          <thead>
            <tr>
              {[
                "Dam", "Runners", "Runs", "Winners", "Wins", "WinPercent_", 
...
const ComparisonTable = ({ entry1, entry2 }) => {
  if (!entry1) return null; // Prevent rendering if no Dam is selected
        ...
        <Table bordered responsive>
          <thead>
            <tr>
              ...
              <th className="text-center">{entry2 ? entry2.Sire : "Entry 2 (Select a Dam)"}</th>
            </tr>
          </thead>
          <tbody>
...
const DamProfile = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/dam_profile?${params.toString()}`);
        ...
        <Row>
          <Col>
            <Card className="shadow border-0 p-4">
              <CardBody>
                <h3 className="text-center mb-4">Dam Profile Search</h3>
                ...
                {/* Display Radar Chart & Comparison Table when a Dam is selected */}
                {selectedEntry1 && (
                  <Row className="mt-4">
                    <Col md={6}>
