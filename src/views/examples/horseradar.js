...
const D3RadarChart = ({ entry1Data, entry2Data }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">Horse Performance Comparison</h4>
...
const ReportTable = ({ tableData, title, currentPage, setCurrentPage, totalPages, onSelectRow }) => {
  ...
  return (
    <Card className="shadow border-0">
      <CardBody>
        <h4 className="text-center mb-3">{title || "Horse Report Table"}</h4>
        <Table responsive className="align-items-center">
          <thead>
            <tr>
              {[
                "Horse", "Runners", "Runs", "Winners", "Wins", "WinPercent_", 
...
const ComparisonTable = ({ entry1, entry2 }) => {
  if (!entry1) return null; // Prevent rendering if no Horse is selected
        ...
        <Table bordered responsive>
          <thead>
            <tr>
              ...
              <th className="text-center">{entry2 ? entry2.Sire : "Entry 2 (Select a Horse)"}</th>
            </tr>
          </thead>
          <tbody>
...
const HorseRadar = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/selected_horses?${params.toString()}`);
        ...
        <Row>
          <Col>
            <Card className="shadow border-0 p-4">
              <CardBody>
                <h3 className="text-center mb-4">Horse Profile Search</h3>
                ...
                <FormGroup>
                  ...
                  >
                    <option value="entry1">Horse 1</option>
                    <option value="entry2">Horse 2</option>
                  </Input>
                </FormGroup>
                ...
                {/* Display Radar Chart & Comparison Table when a Horse is selected */}
                {selectedEntry1 && (
                  <Row className="mt-4">
                    <Col md={6}>
